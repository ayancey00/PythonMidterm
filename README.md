...
If the file option is selected, the program reads a text file such as basic.txt, which contains rows of 0s and 1s by separate spaces. A value of 1 represents a live cell and 0 represents a dead cell. The program checks that the pattern is rectangular and then centers it onto the main grid before the simulation begins. Any custom pattern file must be placed in the same folder as main.py.

The simulation runs on a fixed 60x60 grid. For each generation the program counts the eight neighboring cells of every position. Wrap around boundary conditions are implemented using the modulo indexing, meaning cells that move past one edge of the grid reappear on the opposite side.
